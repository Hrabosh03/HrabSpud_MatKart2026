...
\begin{figure}[htbp]
    ...
    \caption{Výpočet délky stěnové úhlopříčky $u_s$, poloměru $r_s$ kružnice opsané a poloměru $\rho_s$ kružnice vepsané pětiúhelníku, zdroj: Bayer (2026).}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Čtyřboký jehlan $EB_1J_1LD$ a trojúhelník $DJ_1L$, zdroj: Bayer (2026).}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Řez dvanáctistěnem rovinou $0AF$ odpovídající rovině základního poledníku se znázorněním určovaných parametrů, zdroj: Bayer (2026).}
\end{figure}
...
\hline
\textbf{dvanáctistěn} & $P = [58{,}2825^\circ, 72^\circ]$ & $K_1 = [26{,}5651^\circ, 0^\circ]$ \\
\hline
\textbf{dvacetistěn} & $A = [36^\circ, 31{,}7175^\circ]$ & $S_1 = [52{,}6226^\circ, 36^\circ]$ \\
\hline
\end{tabular}
...
Souřadnice kartografického pólu jsou nutné k přepočtení souřadnic analyzovaného bodu do obecné polohy určené tímto pólem. Pro hodnocení zkreslení byl u obou těles zvolen bod ležící na hraně příslušné plošky.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c||c|c|}
        ...
        \hline\hline
        měřítko $m_p$ v poledníku & 3.618026 & 1.145897 \\
        \hline
        měřítko $m_r$ v rovnoběžce & 1.902111 & 1.070466 \\
        \hline
        poloosa $a$ & 3.618026 & 1.145897 \\
        \hline
        poloosa $b$ & 1.902111 & 1.070466 \\
        \hline
        úhel $\omega'$ mezi obrazem poledníku a rovnoběžky & $90^\circ\, 0'\, 0''$ & $90^\circ\, 0'\, 0''$ \\
        \hline
        maximální úhlové zkreslení $\Delta\omega$ & $36^\circ\, 13'\, 12''$ & $3^\circ\, 54'\, 4''$ \\
        \hline
        měřítko ploch $P$ & 6.881887 & 1.226644 \\
        \hline
        meridiánová konvergence $c$ & $54^\circ\, 0'\, 0''$ & $0^\circ\, 0'\, 0''$ \\
        \hline
    \end{tabular}
    \caption{Výsledné vypočtené hodnoty parametrů gnomonické projekce (prostředí MATLAB).}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c||c|c|}
        \hline
         & \textbf{dvanáctistěn} & \textbf{dvacetistěn} \\
        \hline\hline
        měřítko $m_p$ v poledníku & 1.902111 & 1.145897 \\
        \hline
        měřítko $m_r$ v rovnoběžce & 3.618026 & 1.070466 \\
        \hline
        poloosa $a$ & 3.618026 & 1.145897 \\
        \hline
        poloosa $b$ & 1.902111 & 1.070466 \\
        ...
        \hline
    \end{tabular}
    \caption{Výsledné hodnoty parametrů gnomonické projekce získané ze SW Proj.4.}
    \label{tab:vlastnosti_proj4}
\end{table}

Při porovnání vypočtených hodnot s daty ze softwaru Proj.4 se prokazuje naprostá shoda ve všech stěžejních parametrech zkreslení (plošné měřítko, maximální úhlové zkreslení, velikosti poloos i meridiánová konvergence). Jediným specifikem u analyzovaného bodu dvanáctistěnu je přesné prohození hodnot délkových měřítek $m_p$ a $m_r$ mezi lokální a geografickou soustavou. To je matematicky korektní jev způsobený tím, že lokální matematická síť je v tomto bodě vůči celosvětové geografické síti orientována s fázovým posunem, avšak celková elipsa zkreslení zůstává prostorově zcela identická. U dvacetistěnu k tomuto jevu nedochází a shodují se absolutně i obě délková měřítka, jelikož meridiánová konvergence je v daném bodě nulová a obě sítě tak splývají.

Z výsledků je nyní také zřetelně patrný rozdíl ve velikosti zkreslení mezi oběma tělesy v jejich analyzovaných bodech. Vzhledem k odlišné sférické vzdálenosti vybraných bodů od jejich kartografických pólů (středů plošek) nabývá dvanáctistěn v tomto uspořádání logicky výrazně větších hodnot plošného i úhlového zkreslení než dvacetistěn.
