...
Ve funkci \texttt{graticule()}, která slouží ke konstrukci poledníků a rovnoběžek, jsou pomocí metody \texttt{arange()} vygenerovány vektory s definovaným úhlovým krokem ($5^\circ$ pro hlavní síť a jemnější $0{,}5^\circ$ pro plynulé zakřivení linií). Základní výpočetní prostor pro plošnou analýzu států EU je vymezen parametry $u$ (zeměpisná délka) v rozsahu $\langle -12^\circ, 35^\circ \rangle$ a $v$ (zeměpisná šířka) v rozsahu $\langle 34^\circ, 72^\circ \rangle$. Tento prostor je vytvořen pomocí metody \texttt{linspace()}, která vygeneruje vektory 100 rovnoměrně rozložených hodnot mezi hraničními souřadnicemi, což odpovídá výpočetnímu kroku přibližně $0{,}47^\circ$ pro $u$ a $0{,}38^\circ$ pro $v$. Tyto 1D vektory jsou následně pomocí metody \texttt{meshgrid()} transformovány do 2D matic \texttt{latg} a \texttt{long}.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1 \linewidth, trim= 5 5 5 70, clip]{bonne.png}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1 \linewidth, trim= 5 5 5 70, clip]{mercsans.png}
    \caption{Mercator-Sansonovo zobrazení}
\end{figure}

\newpage
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\linewidth, trim= 5 5 5 70, clip]{eck5.png}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\linewidth, trim= 5 5 5 70, clip]{wintri.png}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\linewidth, trim= 5 5 5 70, clip]{hammer.png}
    \caption{Hammerovo zobrazení}
\end{figure}

\newpage
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\linewidth, trim= 5 5 5 70, clip]{lcc.png}
    \caption{LCC zobrazení}
\end{figure}
